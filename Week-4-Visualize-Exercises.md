...
ggplot(data = mpg) +
  ...
  labs(x = "Engine Displacement", y = "Highway MPG", color = "Vehicle Class", title = "Big engines have lower HWY MPG") + 
  theme_light()
```
...
ggplot(data = mpg) + 
  ...
  guides(fill = "none") + 
  labs(x = "class of vehicle", y = "Number of vehicles in sample") + 
  theme_classic()
```
